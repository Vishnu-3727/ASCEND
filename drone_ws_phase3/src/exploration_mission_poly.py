...
# ── Arena configuration (Phase 3 — loaded from JSON via ARENA_CONFIG env) ─
# Polygon is interpreted in the EKF LOCAL frame.
# With EKF2_EV_CTRL=0 (pure optical flow), EKF starts at (0,0) at the drone's
# spawn point. So the polygon should be expressed in coords where the spawn
# (== base station) is at (0,0). For backward compatibility, falls back to
# the phase-2 rectangular arena if no config is set.
import json as _json
import os as _os

_ARENA_DEFAULT = {
    'name':        'rect_baseline_default',
    'polygon':     [(-0.80, -0.80), (9.868, -0.80),
                    ( 9.868, 6.820), (-0.80,  6.820)],
    'poly_margin': 0.50,
    'base':        [0.0, 0.0],
}

_arena_path = _os.environ.get('ARENA_CONFIG')
if _arena_path and _os.path.exists(_arena_path):
    with open(_arena_path) as _f:
        _ARENA = _json.load(_f)
    print(f'[mission] loaded arena config: {_arena_path} ({_ARENA["name"]})')
else:
    _ARENA = _ARENA_DEFAULT
    print('[mission] ARENA_CONFIG not set — using rect_baseline default')

BASE_X: float = float(_ARENA['base'][0])
BASE_Y: float = float(_ARENA['base'][1])
ARENA_POLYGON: list[tuple[float, float]] = [
    (float(p[0]), float(p[1])) for p in _ARENA['polygon']
]
POLY_MARGIN: float = float(_ARENA.get('poly_margin', 0.50))
